...
import risk_environment as risk #the environment
import agent #agent.py the BaseAgent
import random
import config #config.py the settings config
import play #play.py the gui
...
def config_test():
    ...
    trade_vals = config.console_trade_vals()

    return players, deal, order, trade_vals, steal_cards

def assign_territories_test(players=6, deal=True, order=[0,1,2,3,4,5], trade_vals=[4,6,8,10,15,20], steal_cards=True):
    '''
    testing the assigning of territories done at game start
    Optionally you can pass it the outcome of config_test()
    '''
    env = risk.Risk(order,trade_vals, steal_cards)
...
def parse_test():
    ...
    env = risk.Risk([0,1,2,3,4,5],[4,6,8,10,15,20], True)
